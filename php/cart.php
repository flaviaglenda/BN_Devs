...
if (isset($_GET['id']) || isset($_SESSION['id'])) {
    ...
    if ($result->num_rows === 0) {
        ...
        if ($result->num_rows === 0) {
            echo error_json(6, 'Usuário não encontrado.', 'ID procurado: ' . $id);
            exit();
        }
    ...
    switch ($action) {
        ...
        case 'purchase':
            $query = "INSERT INTO compras (id_usuario, descricao, status) VALUES (?, ?, ?)";
            $statement = $connection->prepare($query);

            $status = 'aguardando';
            if (isset($_GET['descricao'])) {
                $descricao = $_GET['descricao'];
                $statement->bind_param('iss', $id, $descricao, $status);
            } else {
                $descricao = null;
                $statement->bind_param('iis', $id, $descricao, $status);
            }
            ...
            $purchase = $statement->insert_id;

            foreach ($items->itens as $value) {
                $query = "UPDATE itens SET id_ligacao = ?, ligacao = 'compra' WHERE id_ligacao = ?";

                $statement = $connection->prepare($query);
                $statement->bind_param('ii', $purchase, $cart['id']);
                $statement->execute();
